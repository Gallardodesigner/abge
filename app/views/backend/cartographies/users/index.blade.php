...
@section("nameview")
Todas as Usuarios de Cartografía
@stop
                ...
                <div class="widgetbox">
                    <div class="headtitle">
                        <div class="btn-group">
                            ...
                            <a href="{{ $route }}/create" class="btn">Adicionar Usuario</a>
                        </div>
                        <h4 class="widgettitle">Todas as Usuarios de Cartografía</h4>
                    </div>
                    <table id="dyntable" class="table table-bordered responsive">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="head0" width="60%">Correo</th>
                                <th class="head0" width="60%">Usuario</th>
                                <th class="head0" width="60%">Publicações</th>
                                <th class="head0" style="width:40%">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if(isset($cartography_users))
                                @foreach($cartography_users as $cartography_user)
                                    <tr class="gradeX">
                                      <td class="aligncenter"><span class="center">
                                        <input type="checkbox" />
                                      </span></td>
                                        <td>{{$cartography_user->email}}</td>
                                        <td>{{ $cartography_user->username }}</td>
                                        <td>{{count($cartography_user->cartographies)}}</td>
                                        <td class="center">
                                            <a href="{{ $route }}/update/{{$cartography_user->id}}" class="btn btn-warning alertwarning" style="color:#FFF !important;"><i class="iconfa-edit" style="color:#FFF;margin-right:10px;"></i>Editar</a>
                                            <a data-id="{{$cartography_user->id}}" data-action="delete" class="btn confirmbutton btn-danger alertdanger" style="color:#FFF !important;"><i class="iconfa-trash" style="color:#FFF;margin-right:10px;"></i>Eliminar</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
